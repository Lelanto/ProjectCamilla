...
class Bot(object):
                    ...
                    if countDown and 'hostile' in countDown.get('class', ''):
                        ...
                        if not attack_id or attack_id in [a.id for a in self.active_attacks]:
                            ...
                            try:
                                ...
                                self.logger.info(tr.find('td', 'sendMail').find('a').get('href'))
                                self.logger.info(tr.find('td', 'sendMail').find('a').get('title'))
                                if destCoords:
                                ...
                                url = 'https://api.telegram.org/bot476138234:AAHnkCs7MCZMYUb6KPaJf0l6ryVinrNXWsc/sendMessage?chat_id='+str(chatIdTelegram)+'&text=ATTACCO%20IN%20CORSO%0AGIOCATORE:%20' + str(
                                    tr.find('td', 'sendMail').find('a').get(
                                        'title')) + '%0ACoordinate%20di%20partenza:%20' + str(
                                    coordsOrigin) + '%0ACoordinate%20di%20arrivo:%20' + str(
                                    destCoords) + '%0AOrario%20di%20arrivo:%20' + str(
                                    arrivalTime) + '%0ANavi%20in%20arrivo:%20' + str(detailsFleet)
    ...
    def farm(self):
        ...
        from_planet = options['farming']['from_planet']
